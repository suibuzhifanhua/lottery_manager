...
MODEL_PATH = os.path.join(MODEL_DIR, "dlt_mlp_model.pkl")
SCALER_PATH = os.path.join(MODEL_DIR, "dlt_scaler.pkl")
...
def compute_features(history):
    ...
    # 奇偶特征
    red_odd = sum(1 for i in range(1, 36) if i % 2 == 1)
    red_even = 35 - red_odd
    
    # 大小特征
    red_large = sum(1 for i in range(1, 36) if i > 17)
    red_small = 35 - red_large
