"""
大乐透 GRU 序列预测模块
使用 PyTorch GRU 神经网络进行序列预测
规则：前区1-35选5个 + 后区1-12选2个
"""
import os
import sys
import numpy as np
import torch
import torch.nn as nn
import joblib

# 设置路径（支持打包后运行）
if getattr(sys, 'frozen', False):
    BASE_DIR = sys._MEIPASS
    APP_DIR = os.path.dirname(sys.executable)
else:
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    APP_DIR = BASE_DIR

# 大乐透规则
RED_COUNT  = 35   # 前区：1-35，选5个
BLUE_COUNT = 12   # 后区：1-12，选2个

# 数据库使用 APP_DIR（用户数据目录），模型使用 BASE_DIR（打包资源目录）
DB_PATH = os.path.join(APP_DIR, "ssq.db")
MODEL_DIR = os.path.join(BASE_DIR, "models")
MODEL_PATH = os.path.join(MODEL_DIR, "dlt_gru_model.pkl")


def _calc_next_issue(last_issue):
    """根据最后一期推算下一期号"""
    try:
        s = str(last_issue)
        if len(s) == 7:
            year = int(s[:4])
            num = int(s[4:])
        else:
            year = int(s[:2])
            num = int(s[2:])
        if num >= 180:
            year += 1
            num = 1
        else:
            num += 1
        if len(s) == 7:
            return f"{year:04d}{num:03d}"
        else:
            return f"{year:02d}{num:03d}"
    except:
        return str(int(last_issue) + 1)


def _compute_gap(history):
    """计算遗漏值"""
    n = len(history)
    red_gap = {i: n for i in range(1, RED_COUNT + 1)}
    blue_gap = {i: n for i in range(1, BLUE_COUNT + 1)}
    red_found = set()
    blue_found = set()
    
    for offset, rec in enumerate(reversed(history)):
        gap = offset
        reds = {rec[f"red{i}"] for i in range(1, 6)}
        blues = {rec[f"blue{i}"] for i in range(1, 3)}
        
        for r in reds:
            if r not in red_found:
                red_gap[r] = gap
                red_found.add(r)
        for b in blues:
            if b not in blue_found:
                blue_gap[b] = gap
                blue_found.add(b)
        
        if len(red_found) == RED_COUNT and len(blue_found) == BLUE_COUNT:
            break
    
    return red_gap, blue_gap


def compute_sequence_features(history, seq_len=10):
    """计算序列特征"""
    n = len(history)
    if n < seq_len:
        return np.zeros(seq_len * (RED_COUNT + BLUE_COUNT))
    
    red_freq = {i: 0 for i in range(1, RED_COUNT + 1)}
    blue_freq = {i: 0 for i in range(1, BLUE_COUNT + 1)}
    
    for rec in history[-seq_len:]:
        for i in range(1, 6):
            red_freq[rec[f"red{i}"]] += 1
        for i in range(1, 3):
            blue_freq[rec[f"blue{i}"]] += 1
    
    red_vals = [red_freq[i] / seq_len for i in range(1, RED_COUNT + 1)]
    blue_vals = [blue_freq[i] / seq_len for i in range(1, BLUE_COUNT + 1)]
    
    return np.array(red_vals + blue_vals, dtype=np.float32)


def create_training_data(history, seq_len=10):
    """创建序列训练数据"""
    X = []
    y_red = []
    y_blue = []
    
    for i in range(seq_len, len(history)):
        X.append(compute_sequence_features(history[i-seq_len:i], seq_len))
        
        # 输出：第i期的前区和后区
        target_reds = [history[i][f"red{j}"] for j in range(1, 6)]
        target_blues = [history[i][f"blue{j}"] for j in range(1, 3)]
        
        # 前区标签：one-hot
        red_label = np.zeros(RED_COUNT, dtype=np.float32)
        for r in target_reds:
            if 1 <= r <= RED_COUNT:
                red_label[r - 1] = 1
        y_red.append(red_label)
        
        # 后区标签
        blue_label = np.zeros(BLUE_COUNT, dtype=np.float32)
        for b in target_blues:
            if 1 <= b <= BLUE_COUNT:
                blue_label[b - 1] = 1
        y_blue.append(blue_label)
    
    return np.array(X), np.array(y_red), np.array(y_blue)


class GRUPredictor(nn.Module):
    """GRU 预测器"""
    def __init__(self, input_size, hidden_size=128, num_layers=2):
        super().__init__()
        self.gru = nn.GRU(input_size, hidden_size, num_layers, batch_first=True, dropout=0.2)
        self.fc_red = nn.Linear(hidden_size, RED_COUNT)
        self.fc_blue = nn.Linear(hidden_size, BLUE_COUNT)
    
    def forward(self, x):
        # x: (batch, seq_len, input_size) -> 需要变成 (seq_len, batch, input_size)
        x = x.unsqueeze(1)  # 添加序列维度
        gru_out, _ = self.gru(x)
        last_out = gru_out[:, -1, :]  # 取最后一个时间步
        red_out = torch.sigmoid(self.fc_red(last_out))
        blue_out = torch.sigmoid(self.fc_blue(last_out))
        return red_out, blue_out


def load_dlt_history():
    """加载大乐透历史数据"""
    import sqlite3
    conn = sqlite3.connect(DB_PATH)
    try:
        c = conn.cursor()
        c.execute("""
            SELECT issue, red1, red2, red3, red4, red5, blue1, blue2
            FROM dlt_history ORDER BY issue ASC
        """)
        rows = c.fetchall()
    finally:
        conn.close()
    return [{"issue": r[0], "red1": r[1], "red2": r[2], "red3": r[3], "red4": r[4], "red5": r[5],
             "blue1": r[6], "blue2": r[7]} for r in rows]


def train_model(db_path=None, seq_len=10):
    """训练 GRU 模型"""
    if db_path is None:
        db_path = DB_PATH
    
    print("开始训练大乐透 GRU 序列模型...")
    print("=" * 50)
    
    history = load_dlt_history()
    if len(history) < seq_len + 10:
        print(f"数据不足: {len(history)} 期，需要至少 {seq_len + 10} 期")
        return None
    
    X, y_red, y_blue = create_training_data(history, seq_len)
    
    print(f"训练数据: {X.shape[0]} 样本, 特征维度: {X.shape[1]}")
    
    # 转换为 PyTorch 张量
    X_tensor = torch.FloatTensor(X)
    y_red_tensor = torch.FloatTensor(y_red)
    y_blue_tensor = torch.FloatTensor(y_blue)
    
    # 创建模型
    input_size = RED_COUNT + BLUE_COUNT
    model = GRUPredictor(input_size)
    
    criterion = nn.BCELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
    
    # 训练
    model.train()
    epochs = 50
    batch_size = 32
    
    for epoch in range(epochs):
        total_loss = 0
        for i in range(0, len(X_tensor), batch_size):
            batch_x = X_tensor[i:i+batch_size]
            batch_red = y_red_tensor[i:i+batch_size]
            batch_blue = y_blue_tensor[i:i+batch_size]
            
            optimizer.zero_grad()
            pred_red, pred_blue = model(batch_x)
            loss = criterion(pred_red, batch_red) + criterion(pred_blue, batch_blue)
            loss.backward()
            optimizer.step()
            total_loss += loss.item()
        
        if (epoch + 1) % 10 == 0:
            print(f"Epoch {epoch+1}/{epochs}, Loss: {total_loss:.4f}")
    
    # 保存模型
    os.makedirs(MODEL_DIR, exist_ok=True)
    torch.save(model.state_dict(), MODEL_PATH)
    print(f"模型已保存到 {MODEL_PATH}")
    
    return model


def load_model():
    """加载模型"""
    if os.path.exists(MODEL_PATH):
        input_size = RED_COUNT + BLUE_COUNT
        model = GRUPredictor(input_size)
        model.load_state_dict(torch.load(MODEL_PATH))
        model.eval()
        return model
    return None


def predict_with_model(db_path=None, seq_len=10):
    """使用模型预测"""
    if db_path is None:
        db_path = DB_PATH
    
    model = load_model()
    
    if model is None:
        print("模型不存在，开始训练...")
        model = train_model(db_path, seq_len)
        if model is None:
            return None
    
    history = load_dlt_history()
    if len(history) < seq_len:
        print(f"数据不足: {len(history)} 期")
        return None
    
    # 用全部历史数据计算特征
    X = compute_sequence_features(history[-seq_len:], seq_len)
    X_tensor = torch.FloatTensor(X).unsqueeze(0)
    
    model.eval()
    with torch.no_grad():
        red_proba, blue_proba = model(X_tensor)
    
    red_proba = red_proba.numpy()[0]
    blue_proba = blue_proba.numpy()[0]
    
    # 选择概率最高的5个前区
    top_red_indices = np.argsort(red_proba)[::-1][:5]
    top_reds = sorted([int(i + 1) for i in top_red_indices])
    
    # 选择概率最高的2个后区
    top_blue_indices = np.argsort(blue_proba)[::-1][:2]
    top_blues = sorted([int(i + 1) for i in top_blue_indices])
    
    return {
        'red_balls': top_reds,
        'blue_balls': top_blues,
        'red_probs': red_proba.tolist(),
        'blue_probs': blue_proba.tolist()
    }


def predict_gru(db_path=None):
    """主预测函数 - 供 predict_dlt.py 调用"""
    try:
        result = predict_with_model(db_path)
        
        if result is None:
            return None
        
        history = load_dlt_history()
        red_freq = {i: 0 for i in range(1, RED_COUNT + 1)}
        blue_freq = {i: 0 for i in range(1, BLUE_COUNT + 1)}
        
        for rec in history:
            for i in range(1, 6):
                red_freq[rec[f"red{i}"]] += 1
            for i in range(1, 3):
                blue_freq[rec[f"blue{i}"]] += 1
        
        hot_reds = sorted(red_freq.keys(), key=lambda k: red_freq[k], reverse=True)[:10]
        hot_blues = sorted(blue_freq.keys(), key=lambda k: blue_freq[k], reverse=True)[:5]
        
        next_issue = _calc_next_issue(history[-1]["issue"])
        
        return {
            'next_issue': next_issue,
            'red_balls': result['red_balls'],
            'blue_balls': result['blue_balls'],
            'method': f'GRU序列模型（基于 {len(history)} 期全部历史数据）',
            'detail': {
                '模型': 'GRU循环神经网络',
                '序列长度': 10,
                '隐藏层': 128,
                '层数': 2,
                '输出': 'sigmoid概率',
                '训练方式': '增量训练' if os.path.exists(MODEL_PATH) else '首次训练'
            },
            'red_freq': red_freq,
            'blue_freq': blue_freq,
            'hot_reds': hot_reds,
            'hot_blues': hot_blues,
            'history_used': len(history),
            'gru_result': result
        }
        
    except Exception as e:
        print(f"GRU预测出错: {e}")
        import traceback
        traceback.print_exc()
        return None


if __name__ == "__main__":
    print("测试大乐透 GRU 序列预测")
    print("=" * 50)
    
    result = predict_gru()
    
    if result:
        print(f"\n预测期号: {result['next_issue']}")
        print(f"前区: {result['red_balls']}")
        print(f"后区: {result['blue_balls']}")
        print(f"方法: {result['method']}")
    else:
        print("预测失败")
